...
# Starts the celeborn worker on the machine this script is executed on with DEBUG enabled.
...
# Enable remote debugging on port 5006 (different from master)
...
CELEBORN_JAVA_OPTS="$CELEBORN_JAVA_OPTS -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5006"
...
echo "Starting Worker with DEBUG on port 5006..."
