...
# Starts the celeborn master on the machine this script is executed on with DEBUG enabled.
...
CELEBORN_JAVA_OPTS="$CELEBORN_JAVA_OPTS -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005"
...
echo "Starting Master with DEBUG on port 5005..."
exec "${CELEBORN_HOME}/sbin/celeborn-daemon.sh" start org.apache.celeborn.service.deploy.master.Master "$@"
